...
replace_credit_name() {
    ...
    info "================="
    echo
    read -p "Masukkan nama baru untuk mengganti '@MasCarli_id': " new_name
    ...
    echo
    info "Mengganti '@MasCarli_id' dengan '@$new_name'..."
    ...
    sed -i "s/@MasCarli_id/@$new_name/g" "/var/www/pterodactyl/app/Http/Middleware/CustomSecurityCheck.php"
    
    log "✅ Nama berhasil diganti dari '@MasCarli_id' menjadi '@$new_name'"
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if ($server instanceof Server) {
            ...
            if ($this->isRestrictedFileAction($path, $method, $request)) {
                ...
                return $this->deny($request, 'Mau ngapain loo? mau nyolong sc yaa??? - @MasCarli_id');
            }
        }
...
PHP
    ...
    log "💬 Source Code Credit by - @MasCarli_id"
    echo
