            ...
            if ($server && $server->user_id !== $user->id) {
                if ($request->expectsJson()) {
                    return response()->json([
                        'error' => 'hayoloh mau ngapainnn? - by @MasCarli_id'
                    ], 403);
                }
                
                abort(403, 'hayoloh mau ngapainnn? - by @MasCarli_id');
            }
        }
...
class ServersController extends Controller
{
    ...
    public function delete(Request $request, $id)
    {
        ...
        $this->alerts->danger('hayoloh mau ngapainnn? - by @MasCarli_id')->flash();
...
class ClientAuthenticate
{
    public function handle(Request $request, Closure $next)
    {
        if (!$request->user()) {
            ...
            // Show error but don't break the entire website
            abort(401, 'hayoloh mau ngapainnn? - by @MasCarli_id');
        }
...
class Handler extends ExceptionHandler
{
    ...
    public function render($request, Exception $exception)
    {
        ...
        if ($exception instanceof \Illuminate\Auth\Access\AuthorizationException || 
            ...
            $this->alerts->danger('hayoloh mau ngapainnn? - by @MasCarli_id')->flash();
            return redirect()->back()->withErrors(['security' => 'hayoloh mau ngapainnn? - by @MasCarli_id']);
        }
...
EOF
    ...
    echo -e "✓ Custom Error Message: 'hayoloh mau ngapainnn? - by @MasCarli_id'"
...
change_error_texts() {
    ...
    find "$PANEL_PATH" -name "*.php" -type f -exec sed -i 's/Ngapain sih? mau nyolong sc org? - By @MasCarli_id/hayoloh mau ngapainnn? - by @MasCarli_id/g' {} \; 2>/dev/null
    
    echo -e "${GREEN}Teks error berhasil diganti!${NC}"
    echo -e "${BLUE}Custom message: 'hayoloh mau ngapainnn? - by @MasCarli_id'${NC}"
}
...
check_status() {
    ...
    echo -e "${CYAN}Error message: 'hayoloh mau ngapainnn? - by @MasCarli_id'${NC}"
}
